...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.MyViewHolder> {
    ...
    public MyAdapter(Context con, List<Match> Mdata, List<String> logo){
        this.context = con;
        this.Mdata = Mdata;
        this.logo = logo;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        Picasso.get().load(logo.get(position*2)).into(holder.myImg1);
        Picasso.get().load(logo.get((position*2)+1)).into(holder.myImg2);
    }

    @Override
    public int getItemCount() {
        return (logo.size()+1)/2;
    }
